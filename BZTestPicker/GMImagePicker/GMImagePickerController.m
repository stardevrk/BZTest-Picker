...
- (id)init
{
    if (self = [super init]) {
        ...
        NSError *error;
        FSSecurity *security = [[FSSecurity alloc] initWithPolicy:policy appSecret:@"XQSOOK6NVFFYBAFORLKVXLJBSU" error:&error];
        if (error != nil) {
            NSLog(@"Error instantiating policy object: %@", error.localizedDescription);
            _client = nil;
        } else {
            _client = [[FSClient alloc] initWithApiKey:@"AXkc2JgbsSQVuCZV9w4iBz" security:security];
        }
